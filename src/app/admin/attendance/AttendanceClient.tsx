...
export default function AttendanceClient({
  ...
  const filteredRecords = initialRecords.filter((rec) => {
    ...
    const matchesEmployee = selectedEmployee === 'ALL' || rec.employeeId === selectedEmployee;
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-border">
              {filteredRecords.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-muted-foreground">No attendance records found matching filters.</td>
                </tr>
              ) : (
                    ...
                    <tr key={r.id} className="hover:bg-secondary/15 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="font-bold">{r.employee.name}</p>
                            <p className="text-[9px] text-muted-foreground leading-none mt-0.5">{r.employee.department?.name || 'No Dept'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4 space-y-0.5">
                        {checkOutDate ? (
                          ...
                          <span className="text-[10px] text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md font-semibold animate-pulse">Logged In</span>
                        )}
                      </td>
                      ...
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-extrabold tracking-wider uppercase ${
                          ...
                          r.status === 'LATE' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/10' :
                          r.status === 'HALF_DAY' ? 'bg-indigo-500/10 text-indigo-400' :
                      ...
                      <td className="p-4 text-muted-foreground font-mono">
                        {r.latitude && r.longitude ? (
                          ...
                          <span className="italic text-muted-foreground/60">No GPS data</span>
                        )}
                      </td>
